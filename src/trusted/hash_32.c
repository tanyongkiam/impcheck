...
u64 compute_idx(struct hash_table_32* ht, u64 key) {
    return compute_hash(key) & (ht->capacity-1);
}

bool cell_empty(struct hash_table_32_entry* entry) {
    return entry->key == 0;
}

bool find_entry(struct hash_table_32* ht, u64 key, u64* idx) {
    u64 i = compute_idx(ht, key);
    const u64 orig_idx = i;
    while (i < ht->capacity) {
        struct hash_table_32_entry* entry = &ht->data[i];
        if (cell_empty(entry)) {
...
bool realloc_table(struct hash_table_32* ht) {
    u64 new_capacity = (u64) (ht->growth_factor * ht->capacity);
    //printf("GROW %lu -> %lu\n", ht->capacity, new_capacity);
    struct hash_table_32_entry* old_data = ht->data;
    u64 old_capacity = ht->capacity;
    ht->data = (struct hash_table_32_entry*) trusted_utils_calloc(new_capacity, sizeof(struct hash_table_32_entry));
    if (!ht->data) return false;
    ht->size = 0;
    ht->max_size = (u64) (ht->growth_factor * ht->max_size);
    ht->capacity = new_capacity;
    for (u64 i = 0; i < old_capacity; i++) {
        struct hash_table_32_entry* cell = &old_data[i];
        if (!cell_empty(cell)) {
            if (!hash_table_32_insert(ht, cell->key, cell->val))
                return false;
        }
    }
    free(old_data);
...
bool handle_gap(struct hash_table_32* ht, u64 idx_of_gap) {
    ...
    while (true) {
        ...
        if ((j > i && (k <= i || k > j)) 
            ...
            struct hash_table_32_entry* entry_at_deletion = &ht->data[i];
            struct hash_table_32_entry* entry2move = &ht->data[j];
...
struct hash_table_32* hash_table_32_init(int log_init_capacity) {
    struct hash_table_32* ht = trusted_utils_malloc(sizeof(struct hash_table_32));
    ht->capacity = 1<<log_init_capacity;
    ht->size = 0;
    ht->max_size = (u64) (ht->capacity >> 1);
    ht->growth_factor = 2;
    ht->data = (struct hash_table_32_entry*) trusted_utils_calloc(ht->capacity, sizeof(struct hash_table_32_entry));
    return ht;
}
...
bool hash_table_32_insert(struct hash_table_32* ht, u64 key, u32 val) {
    ...
    struct hash_table_32_entry* entry = &ht->data[idx];
    entry->key = key;
    entry->val = val;
    ht->size++;
    return true;
